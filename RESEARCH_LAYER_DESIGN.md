...
## 6. Discovery and ingestion

**The system finds its own papers.** The owner supplies topics, not documents.
Requiring a curated reading list would defeat the purpose — this layer exists to keep
current with minimal input.

### 6.1 Access reality

**No institutional or paid journal access.** Open access only. That is a hard
constraint on what can be ingested, and it shapes the grading:

- **Full open-access text** — method, sample size and funding declaration are all
  readable, so a claim can reach grade A or B and qualify for auto-activation.
- **Abstract only** — design and sample size are often stated; funding almost never
  is. `funding_independent` is then null, which under §5 fails auto-activation
  criterion 3 automatically. Abstract-derived claims are catalogued and reviewable,
  but they cannot go live unattended. The rule already handles this; no special case
  is needed.

This is a real ceiling on quality, and it is honest to state it rather than pretend
otherwise.
...
Resolve the returned PMID, DOI and PMCID against **Europe PMC**. Europe PMC is the
open-access full-text source, not the species-discovery source: for records with a
PMCID, fetch `fullTextXML` and parse the JATS `<funding-group>` and structured
competing-interest declarations (`<fn-group>` or publisher-specific
`<sec sec-type="COI-statement">`) verbatim. Funding independence is a deterministic
rule over those stored statements. Abstract-only records retain null funding
metadata and fail closed. **DOAJ**, **PLOS**, **BMC Veterinary Research**,
**Frontiers in Veterinary Science** where not already covered. **WSAVA** and
**FEDIAF** guidance documents as standing references.

Nothing outside the allowlist enters. No blogs, no manufacturer white papers, no
press releases, no secondary science journalism.

**Preprints** (bioRxiv, medRxiv and similar) are not peer reviewed. They may be
ingested and catalogued but are capped at grade D and may never auto-activate,
regardless of other metadata.

### 6.3 Topic seeds

The owner defines the *domain*, once, and revises it rarely. Starting set:

canine nutrition; food allergy and adverse food reaction in dogs; food intolerance;
hydrolysed and novel protein diets; canine gut microbiome; dysbiosis; short-chain
fatty acids and gut barrier function; dietary fibre in dogs; elimination diet trials;
food-responsive enteropathy; ingredient-level sensitivity.

Each topic becomes a structured query. Precision comes from the source's own
filters — species terms, MeSH headings, publication type, date range — not from a
model sifting results afterwards. One reconnaissance pass per source establishes the
query shape; it then runs unattended, exactly as the domain-selector pattern works
for food sites.

### 6.4 Pipeline

1. **Search** each topic query against each source on a schedule. Collect candidate
   records with metadata only.
2. **Deduplicate** on DOI, then on title similarity. Skip anything already in
   `research_documents` or already rejected.
3. **Filter** on metadata alone, before any cost is incurred: species, publication
   type, date, retraction status, open-access availability.
4. **Fetch** full text where OA, abstract otherwise. Store in `research_documents`
   with all grading metadata populated from the source's structured fields — never
   inferred by a model.
5. **Chunk and embed** into `research_chunks` (`vector(1536)`).
6. **Rank for relevance** by embedding similarity against the topic centroids.
   Below-threshold documents are catalogued but not sent for claim drafting — this
   is the main cost control.
7. **Draft claims** offline, per §3. Every `supporting_quote` asserted a literal
   substring of its chunk; failures discarded, never repaired. The quote must
   support the claim attached to it, not merely contain its words — the attribution
   doctrine from §11 applies here too.
8. **Grade** deterministically from stored metadata (§5). Auto-activate or queue.

### 6.5 Volume and cost control

An open search returns far more than is useful, and embedding costs real money on a
self-funded project.

- Hard cap on new documents ingested per run.
- Relevance threshold applied *before* claim drafting, which is the expensive step.
- Batch API for drafting; never per-request inference.
- A run reports estimated cost before incurring it on the first pass.
- `system_alerts` raised if the review queue exceeds a threshold — a queue nobody
  can clear is the same failure as an alarm nobody reads.

### 6.6 Cadence and health

Weekly, logged to `cron.job_run_details`. Raise a `system_alerts` row if two
consecutive runs ingest nothing, if the review queue exceeds its threshold, or if
retraction re-checking fails.

**Retraction watch:** re-check stored DOIs monthly against Europe PMC and Crossref.
A retracted document immediately sets all its claims to `superseded` and raises an
alert.
...
1. ~~**Full-text access.**~~ **Answered 2026-07-28** — no institutional or paid
   access. Open access and abstracts only. See §6.1: abstract-derived claims fail
   auto-activation automatically because funding cannot be verified, so the existing
   §5 rule handles it without a special case.
...
5. ~~**Seed corpus.**~~ **Answered 2026-07-28** — there is no owner-curated paper
   list, by design. The owner supplies *topics* (§6.3); the system discovers papers.
   Calibration comes instead from the owner reviewing every claim drafted on the
   first discovery run, before auto-activation is enabled (§13 step 5).
...
3. Discovery: topic queries against Europe PMC and the source allowlist (§6.2–6.4),
   run once manually. Documents ingested, chunked, embedded. Owner reviews what the
   queries actually returned before anything is drafted — this calibrates precision.
4. Claim drafting against that first corpus. Owner reviews every claim — this
   calibrates the extractor and the grading.
5. Auto-activation rule enabled once drafting quality is proven.
6. Scheduled weekly discovery + ingestion, unattended.
